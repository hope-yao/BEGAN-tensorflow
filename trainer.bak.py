        ...
        for epoch in range(5000):
            ...
            for i in tqdm(range(it_per_ep)):
                ...
                result = self.sess.run([self.d_loss_real, self.d_loss_fake, self.d_loss, self.g_loss, self.k_update,
                                        self.klf_mean, self.klr_mean, self.pt_z, self.nom, self.denom], feed_dict)
                print(result)
                import math
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for i in [1]:
                with tf.device('/gpu:%d' % i):
                    with tf.variable_scope('%s_gpu%d' % ("G", i)) as vs_g:
                    ...
                    with tf.variable_scope('%s_gpu%d' % ("D", i)) as vs_d:
                        ...
                        self.d_out, self.D_sub_norm = Decoder(self.y, self.d_z, self.channel, self.z_num,
                                                              self.repeat_num,
                                                              self.conv_hidden_num, self.data_format)
                        # self.d_out, self.D_sub_norm, self.d_zg, self.d_z, self.kl_loss \
                        #     = DiscriminatorCNN(self.y, self.all_x,self.channel,self.z_num,
                        #                        self.repeat_num,self.conv_hidden_num,self.data_format)
                        self.D_var = tf.contrib.framework.get_variables(vs_d)
                        tf.get_variable_scope().reuse_variables()

                    self.pt_z, self.nom, self.denom =  calc_pt_Angular(tf.split(self.d_z,2)[0],self.batch_size)#tf.Variable(0.)#
                    self.klf, self.klr, self.mr, self.mf = calc_eclipse_loss_analy(self.d_z,self.z,n_net)
                    # self.kl_loss = calc_eclipse_loss(self.d_z,self.z,n_net)
                    # self.klf, self.klr = self.kl_loss
                    self.klf_mean = tf.reduce_mean(list2tensor(self.klf))
                    self.klr_mean = tf.reduce_mean(list2tensor(self.klr))
                    self.mr = tf.reduce_mean(list2tensor(self.mr))
                    self.mf = tf.reduce_mean(list2tensor(self.mf))
                    ...
                    self.d_loss_real = 4 * 20 * tf.reduce_mean(tf.abs(list2tensor(self.AE_x_norm) - tf.tile(self.x_norm,[L,1,1,1])))
                    self.d_loss_fake = 4 * 20 * tf.reduce_mean(tf.abs(list2tensor(self.AE_G_norm) - tf.tile(self.G_norm,[L,1,1,1])))
                    self.f_dis = self.klf_mean*0.2 + self.mf*0.2
                    self.r_dis = self.klr_mean*0.2 + self.mr*0.2
                    self.g_loss = self.d_loss_fake + self.f_dis + (1-self.pt_z)
                    self.d_loss = self.d_loss_real - self.k_t * self.d_loss_fake + self.f_dis + self.r_dis
