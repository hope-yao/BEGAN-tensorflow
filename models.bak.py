...
def calc_eclipse_loss(dz,z,N_Subnets):
    ...
    for i in range(N_Subnets):
        ...
        mi_dzf = tf.matmul(tf.transpose(dzf_i, (1, 0)), dzf_i)

        mi_dzr = tf.matmul(tf.transpose(dzr_i, (1, 0)), dzr_i)

        mi_z = tf.matmul(tf.transpose(z_i, (1, 0)), z_i)



        l_f_i = tf.reduce_mean(tf.abs(mi_z-mi_dzf))

        l_f += [tf.expand_dims(l_f_i, 0)]

        l_r_i = tf.reduce_mean(tf.abs(mi_z - mi_dzr))
...
def calc_eclipse_loss_analy(dz,z,N_Subnets):
    ...
    for i in range(N_Subnets):
        ...
        l_f_i = tf.reduce_mean(tf.square(mi_z-mi_dzf))
        ...
        m_f_i = tf.reduce_mean(tf.abs(z_mean - dzr_mean))

        m_f += [tf.expand_dims(m_f_i, 0)]

        m_r_i = tf.reduce_mean(tf.abs(z_mean - dzf_mean))

        m_r += [tf.expand_dims(m_r_i, 0)]



    return  l_f, l_r, m_r, m_f
...
def DiscriminatorCNN(y_data, x, input_channel, z_num, repeat_num, hidden_num, data_format):
    ...
    z = slim.fully_connected(x, z_num*2, activation_fn=None) # times 2 for mean and variance



    dup = 2

    z_g, z_mean, z_log_var = my_sampling(z,n_subnet)

    kl_f, kl_r = calc_kl_loss(z_mean, z_log_var,bs)
    ...
    for net_i in range(n_subnet):
        ...
        out_sub += [out_i[0:bs*2]]



    # out = tf.clip_by_value(out, 0, 1)

    return out, out_sub, z_g, z, (kl_f, kl_r)
...
def calc_pt_Angular(z_d_gen, bs):
    ...
    denom = tf.sqrt(tf.reduce_sum(tf.square(z_d_gen), reduction_indices=[1], keep_dims=True)) + 1e-8
